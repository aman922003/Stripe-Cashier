        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 p-6 rounded shadow">
                <form method="POST" action="{{ route('coupons.store') }}">
                    ...
                    <div id="amount_off_field" class="mb-4 hidden">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="duration" id="duration" class="w-full border-gray-300 rounded mt-1" required>
                            ...
                            <option value="repeating" {{ old('duration') === 'repeating' ? 'selected' : '' }}>Duration in Months</option>
                        </select>
                        @error('duration') <span class="text-red-600">{{ $message }}</span> @enderror
                    </div>

                    <div class="mb-4 hidden" id="duration_in_months_container">
                        ...
                        <input type="number" name="duration_in_months" class="w-full border-gray-300 rounded mt-1"
                               value="{{ old('duration_in_months') }}" min="1" max="100">
                        @error('duration_in_months') <span class="text-red-600">{{ $message }}</span> @enderror
                    </div>

                    <div class="mb-4 flex items-center">
                        <input type="checkbox" name="generate_code" value="1"
                               {{ old('generate_code') ? 'checked' : '' }}>
                        <label class="ml-2 text-gray-700 dark:text-gray-200">Generate Promo Code</label>
                    </div>
